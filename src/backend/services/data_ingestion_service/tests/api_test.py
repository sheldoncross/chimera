...
def test_scrape_news_success(client, mock_ingestion_service):
...
def test_scrape_news_with_start_urls(client, mock_ingestion_service):
...
def test_scrape_news_service_exception(client, mock_ingestion_service):
...
def test_clear_scrape_queue_success(client, mock_ingestion_service):
    """Test successful queue clearing"""
    response = client.get("/api/v1/clear_scrape_queue")
...
def test_clear_scrape_queue_service_exception(client, mock_ingestion_service):
    ...
    with pytest.raises(Exception):
        client.get("/api/v1/clear_scrape_queue")